 ...
 */
System::System(DynaRadar::DynaRadar* pDynaRadar,const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, const int initFr, const string &strSequence):
    mpDynaRadar(pDynaRadar),mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false), mbResetActiveMap(false),
...
void System::radarGetCurrentFrame(Frame &currentFrame,KeyFrame *pReferenceKF)
{
    unique_lock<mutex> lock(mMutexState);
    mpDynaRadar->filterCurrentFrame(currentFrame,pReferenceKF);
}
cv::Mat System::getRadarMask(void)
{
        return mpDynaRadar->getMask();
}
vector<cv::RotatedRect> System::getRadarMaskBoxs(void)
{
        return mpDynaRadar->getMaskBoxs();
}
