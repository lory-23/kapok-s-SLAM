...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    vector<cv::KeyPoint> vMaskedKeys;
    ...
    cv::Scalar odometryColor(255,0,255);

    cv::Scalar ColorMasked(100,0,100);
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i] || vbMap[i])    //绿色点，正常跟踪点
            {
                ...
                if(vbMap[i])
                {
                    if(mvbMasked[i])    cv::rectangle(im,pt1,pt2,ColorMasked);
                    else                               cv::rectangle(im,pt1,pt2,standardColor);
                ...
                {
                    if(mvbMasked[i])   cv::rectangle(im,pt1,pt2,ColorMasked);
                    else                              cv::rectangle(im,pt1,pt2,odometryColor);
                    cv::circle(im,point,2,odometryColor,-1);
                    mnTrackedVO++;
                }
            }

            else if(mvbOutlier[i])                   //标记为外点，跟踪后最后被排除的点
            {
                cv::Point2f pt1,pt2;
                cv::Point2f point;
                if(imageScale != 1.f)
                ...
                else
                {
                    ...
                    if(mvbMasked[i])   cv::rectangle(im,pt1,pt2,ColorMasked);
                    else                              cv::rectangle(im,pt1,pt2,badColor);
                    cv::circle(im,point,2,badColor,-1);
            }

            else if(false)
            // else if(mvbMasked[i])                     //被masked的普通特征点keypoint
            {
                cv::Point2f pt1,pt2;
                cv::Point2f point;
                if(imageScale != 1.f)
                ...
                cv::rectangle(im,pt1,pt2,ColorMasked);
                cv::circle(im,point,2,ColorMasked,-1);
            }

        }
        for(auto box:mBoxs)
        {
            cv::ellipse(im, box.center,cv::Size(box.size.width,box.size.height),box.angle,0, 360,cv::Scalar(0),1) ;
        }
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    // pTracker->mImMask.copyTo(mIm); 
    mvCurrentKeys=pTracker->mCurrentFrame.mvKeys;
    mThDepth = pTracker->mCurrentFrame.mThDepth;
    mvCurrentDepth = pTracker->mCurrentFrame.mvDepth;
    mBoxs = pTracker->mBoxs;
    if(both){
    ...
    mvbMasked = pTracker->mCurrentFrame.mvbMasked;
    mvbOutlier = pTracker->mCurrentFrame.mvbOutlier;
