...
{
    mImGray = im; cv::Mat imMask;
    if(mpSystem->mpDynaRadar==nullptr)
        imMask=cv::Mat::ones(mImGray.size.p[0],mImGray.size.p[1],CV_8U);
    else
        imMask = mpSystem->getRadarMask();
    // Step 1 ：将彩色图像转为灰度图像
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
               ...
               cout<<"track currentFrame outlier_num:"<< outlier_num<<endl;
