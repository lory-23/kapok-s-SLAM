     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    // 记录地图点是否为外点，初始化均为false，表示均不是外点
    mvbOutlier = vector<bool>(N,false);
   mvbMasked = vector<bool>(N,false);
    int count=0; 
    for(int i=0;i<N;i++)
    {
       if( mask.at<uchar>( mvKeysUn[i].pt.y,mvKeysUn[i].pt.x)==0)
       {
            mvbMasked[i]=true;
            count++;
       }
     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mvbOutlier = vector<bool>(N,false);
    mvbMasked = vector<bool>(N,false);
    //  cout<<"ImGray 1:"<<imGray.size.p[1]<<"    0:"<<imGray.size.p[0]<<endl;
    //  cout<<"imMask 1:"<<mask.size.p[1]<<"    0:"<<mask.size.p[0]<<endl;
    int count=0; 
    for(int i=0;i<N;i++)
    {
       if( mask.at<uchar>( mvKeysUn[i].pt.y,mvKeysUn[i].pt.x)==0)
    ...
    // cout<<"init out KP count: "<<count<<endl;
...
void Frame::ComputeBoW()
{
    ...
    if(mBowVec.empty())
    {
        ...
        // cout<<"mDescriptors:  "<<mDescriptors<<endl;
