...
void Tracking::getMask()
{
    if(mpSystem->mpDynaRadar==nullptr)
    {
        mImMask=cv::Mat(mImGray.size.p[0],mImGray.size.p[1],CV_8U,255);
    }
    else
    {
        cv::Mat Mask = cv::Mat(mImGray.size.p[0],mImGray.size.p[1],CV_8U,255);
        mBoxs = mpSystem->getRadarMaskBoxs();
        for(auto box:mBoxs)
        {
            cv::ellipse(Mask, box.center,cv::Size(box.size.width,box.size.height),box.angle,0, 360,cv::Scalar(0),cv::FILLED) ;
        }
        mImMask = Mask;
    }
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    getMask();   //mImMask
    cout<<"maskSize"<<mImMask.size()<<endl;
    // step 1：将RGB或RGBA图像转为灰度图像
    if(mImGray.channels()==3)
    {
        if(mbRGB)
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,mImMask,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,mImMask,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    // mImGray = mImGray*0;
    // _mImGray.copyTo(mImGray,mImMask);
    ...
    else if(mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            mCurrentFrame = Frame(mImGray,mImMask,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray,mImMask,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
    }
    ...
    // cout<<"mCurrentFrame id:  "<<mCurrentFrame.mnId<<endl;
//     if(mpReferenceKF)
//    {
//     cout<<"mpReferenceKF id:  "<<mpReferenceKF->mnId<<endl;
    // mpSystem->radarGetCurrentFrame(mCurrentFrame,mpReferenceKF);
//    }
//    else
//    cout<< "mpReferenceKF is nullptr"<<endl;

    // std::cout <<    "    after: "<<mCurrentFrame.mvKeysUn.size()<<endl;
    // Step 3 ：跟踪
    Track();
...
void Tracking::Track()
{
    ...
    if(mpLocalMapper->mbBadImu)
    {
        ...
        Verbose::PrintMess("KeyFrames In Map  less 10",Verbose::VERBOSITY_DEBUG);//yuanqi+
    ...
    if(mState!=NO_IMAGES_YET)
    {
        if(mLastFrame.mTimeStamp>mCurrentFrame.mTimeStamp)
        {
            ...
            cerr << "ERROR: Frame with a timestamp older than previous frame detected!  " << "current: "<<setprecision(16)<<mCurrentFrame.mTimeStamp<<"last: "<<mLastFrame.mTimeStamp<< endl;
            unique_lock<mutex> lock(mMutexImuQueue);
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        //mpFrameDrawer->Update(this);    // 显示mask标记区域内全部特征点
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                //初始化，在初始化时被雷达动态区域赋为外点
                int aux1=0,aux2=0;
                   for(int i=0; i<mCurrentFrame.N; i++)
                   {
                        if( mCurrentFrame.mvpMapPoints[i])
                            aux1++;
                        if(mCurrentFrame.mvbMasked[i])
                            aux2++;
                   }
               
                cout<<"1、Init: MapPoints"<<aux1<<"  MaskKetPiont"<<aux2<<"  KeyPoint"<<mCurrentFrame.N<<endl;

                // mpFrameDrawer->Update(this); //初步跟踪后更新显示特征点。显示mask标记区域内全部特征点
            ...
            else  // 跟踪不正常按照下面处理
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        // Step 6.5 纯视觉模式则进行重定位。主要是BOW搜索，EPnP求解位姿（3妙后还不行，放弃抢救）
                        // Relocalization
                        bOK = Relocalization();
        ...
        // mpFrameDrawer->Update(this);   //第二次优化后
...
bool Tracking::TrackReferenceKeyFrame()
{
   ...
   //运动跟踪的匹配结果
    ...
    cout<<"2、ReferenceSearch:   MapPoints"<<aux1<<"  Masked"<<aux2<<endl;
...
bool Tracking::TrackWithMotionModel()
{
   ...
   //运动跟踪的匹配结果
    ...
    cout<<"2、motionSearch:   MapPoints"<<aux1<<"  Masked"<<aux2<<endl;

    // Optimize frame pose with all matches
    // Step 4：利用3D-2D投影关系，优化当前帧位姿
    Optimizer::PoseOptimizationWithMasked(&mCurrentFrame);
    ...
    //             aux2++;
    //     }
    // cout<<"3、motionPoseOpt:   MapPoints"<<aux1<<"  Outlier"<<aux2<<endl;
    mpFrameDrawer->Update(this); //第一次优化后。
...
bool Tracking::TrackLocalMap()
{
    ...
    cout<<"4、SearchLocalPoints: MapPoints"<<aux1<<"  Outlier"<<aux2<<endl;
    ...
    cout<<"5、localPoseOpt: MapPoints"<<aux1<<"  Outlier"<<aux2<<endl;
    mnMatchesInliers = 0;
