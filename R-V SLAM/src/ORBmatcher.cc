...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
    {
        ...
        DBoW2::FeatureVector::const_iterator KFit = vFeatVecKF.begin();   //关键帧的词袋特征向量
        DBoW2::FeatureVector::const_iterator Fit = F.mFeatVec.begin();      //普通帧词袋向量
...
////////////////////////////////////////////////////////////////////
  ...
  /**
     ...
     * @return int                  成功匹配的数量
     */
    int ORBmatcher::MatchByBoW(KeyFrame* pKF,Frame &F,  std::vector<int>& mvIniMatches)
    {
        // 和普通帧F特征点的索引一致
    //    mvIniMatches  = vector<int>(F.N,static_cast<int>(NULL));
       mvIniMatches  = vector<int>(F.N,-1);
        ...
        // cout<<"vFeatVec KF size:"<<vFeatVecKF.size()<<endl ;
        // cout<<"vFeatVec F size:"<<F.mFeatVec.size()<<endl ;

        // cout <<"KFit =? KFend "<< (KFit==KFend)  <<endl;
        // cout <<"Fit =? Fend "<< (Fit==Fend)  <<endl;
        while(KFit != KFend && Fit != Fend)
        {
            ...
            if(KFit->first == Fit->first) 
            {         
                // cout<<"get same node:  "<<    Fit->first<<endl;
                // second 是该node内存储的feature index
                const vector<unsigned int> vIndicesKF = KFit->second;
                const vector<unsigned int> vIndicesF = Fit->second;
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    // 关键帧该节点中特征点的索引
                    const unsigned int realIdxKF = vIndicesKF[iKF];
                    // cout<< "KF keypoint id: "  << realIdxKF  <<endl;
                    // 取出关键帧KF中该特征对应的描述子
                    const cv::Mat &dKF= pKF->mDescriptors.row(realIdxKF); 
                    ...
                    if(bestDist1<=TH_LOW)
                    {
                        ...
                        if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                        {
                            // Step 4.3：记录成功匹配特征点的对应的特征点(来自关键帧)
                            mvIniMatches[bestIdxF]=realIdxKF;
        ...
        if(mbCheckOrientation)
        {
            ...
            for(int i=0; i<HISTO_LENGTH; i++)
            {
                ...
                for(size_t j=0, jend=rotHist[i].size(); j<jend; j++)
                {
                    mvIniMatches[rotHist[i][j]]=static_cast<int>(NULL);
                    nmatches--;
                }
            }
        }
...
int ORBmatcher::MatchByWindow(KeyFrame &F1, Frame &F2, vector<int> &vnMatches21, int windowSize)
    {
        int nmatches=0;
        // F1中特征点和F2中匹配关系，注意是按照F1特征点数目分配空间
        vector<int> vnMatches12 (F1.mvKeysUn.size(),-1);
        ...
        vnMatches21 = vector<int> (F2.mvKeysUn.size(),-1);
        ...
        for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
        {
            ...
            vector<size_t> vIndices2 = F2.GetFeaturesInArea(F2.mvKeysUn[i1].pt.x,F2.mvKeysUn[i1].pt.y, windowSize,level1,level1);
        ...
        for(int i=0; i<LastFrame.N; i++)
        {
            ...
            if(pMP)
            {
                if(!LastFrame.mvbOutlier[i])
                {
                    ...
                    {
                        const size_t i2 = *vit;         //i2为当前帧候选匹配特征点id
                        ...
                        if(CurrentFrame.Nleft == -1 && CurrentFrame.mvuRight[i2]>0)      //单目不考虑
                        {
