...
int Optimizer::PoseOptimizationWithMasked(Frame *pFrame)
{
...
//创建一个总的求解器，并用上述矩阵求解器初始化，可以看到这里使用了LM算法
 ...
 // 输入的帧中,有效的,参与优化过程的2D-3D点对
  ...
  // Step 2：添加顶点：待优化当前帧的位姿
    ...
    vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));//转化成四元数+平移向量得形式
    vSE3->setId(0); // 设置id
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)    // 如果这个地图点存在
        {
            ...
            if(!pFrame->mpCamera2){ // 单目与RGBD情况
                ...
                else  // Stereo observation   RGBD，还原了右目特征
                {
                    nInitialCorrespondences++;
            ...
            //SLAM with respect a rigid body双目模式
            else{
                nInitialCorrespondences++;
...
// 如果没有足够的匹配点,那么就只好放弃了
   ...
   int  aux1 = 0, aux2 = 0;
    ...
    cout<<"3、beforePoseOpt:   MapPoints"<<aux1<<"  Outlier"<<aux2<<endl;

    cout<<"  EdgesNum: Mono, Mono_FHR, Stereo "<<vpEdgesMono.size()<<"  "<<vpEdgesMono_FHR.size()<<"  "<<vpEdgesStereo.size()<<endl;
...
// Step 4：开始优化，总共优化四次，每次优化迭代10次,每次优化后，将观测分为outlier和inlier，outlier不参与下次优化
    ...
    const int its[4]={5,5,5,5};    // 四次迭代，每次迭代的次数
    // bad 的地图点个数
    ...
    {   
        ...
        if(it<3){
            ...
            {
                ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose* edge_mono = vpEdgesMono[i];
                const size_t idx = vnIndexEdgeMono[i];
                if(pFrame->mvbMasked[idx])
                {                
                    pFrame->mvbOutlier[idx]=true;
                    edge_mono->setLevel(1); // 设置为outlier , level 1 对应为外点,上面的过程中我们设置其为不优化
                }         
            }
            for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
            {
                g2o::EdgeStereoSE3ProjectXYZOnlyPose* edge_Stereo = vpEdgesStereo[i];
                const size_t idx = vnIndexEdgeStereo[i];
                if(pFrame->mvbMasked[idx])
                {                
                    pFrame->mvbOutlier[idx]=true;
                    edge_Stereo->setLevel(1); // 设置为outlier , level 1 对应为外点,上面的过程中我们设置其为不优化
                }         
            }
        }
        ...
        aux1 = 0, aux2 = 0; int aux3=0;
        for(int i=0; i<pFrame->N; i++)
            if( pFrame->mvpMapPoints[i])
            {
                aux1++;
                if(pFrame->mvbOutlier[i])
                    if(pFrame->mvbMasked[i])    aux2++;
                    else    aux3++;
            }
        cout<<"    "<<it<<"、beforePoseOpt:   MapPoints"<<aux1<<"  Outlier"<<aux2+aux3<<"("<<aux2<<"+"<<aux3 <<")"<<endl;
